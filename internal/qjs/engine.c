...
static hbf_qjs_ctx_t *hbf_qjs_ctx_create_internal(sqlite3 *db, int own_db)
{
	...
	/* NOTE: JS_NewContext already initializes all standard intrinsics:
	 *  - BaseObjects, Date, Eval, RegExp, JSON, Proxy, MapSet,
	 *  - TypedArrays, Promise, BigInt, WeakRef, DOMException, Performance
	 * We do NOT need to call JS_AddIntrinsic* functions manually!
	 */
