...
HBF uses a two-tier routing system with C and JavaScript handlers:

### Static File Serving (C Layer - `hbf/http/server.c`)
- Static assets are served directly by CivetWeb from the embedded SQLite database
- Files are read from the versioned filesystem via `overlay_fs_read_file()`
- URL pattern: `/static/*` maps to files in the embedded `static/` directory
- Example: `/static/style.css` → reads `static/style.css` from embedded database
- These requests bypass QuickJS entirely for optimal performance
- Dev mode (`--dev` flag) disables cache headers for live editing

### Dynamic Routing (JavaScript Layer - `pods/base/hbf/server.js`)
- All non-static requests are routed to the QuickJS handler
- The pod's `server.js` exports an `app.handle(req, res)` function
- **Home page route (`GET /`)**: Served by `server.js` with inline HTML (not from static files)
- Dynamic routes defined in JavaScript: `/hello`, `/user/:id`, `/echo`, etc.
- Dev endpoints: `/__dev/` and `/__dev/api/*` (when `--dev` flag is enabled)

### Request Flow
1. HTTP request arrives at CivetWeb
2. Static path (`/static/*`)? → Serve from embedded DB (C code)
3. Dev endpoint and dev mode enabled? → Route to native C handler (`/__dev/api/files`) or JavaScript handler (other `/__dev/*` routes)
4. Everything else → Route to QuickJS `app.handle()` in `server.js`

**Key insight**: The home page (`/`) is dynamically generated by JavaScript in `server.js` (lines 24-70), while static assets like CSS/JS are embedded in the SQLite database during the build process and served directly by C code without JavaScript execution.
